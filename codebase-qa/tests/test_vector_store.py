...
@pytest.fixture
def mock_embeddings():
    with patch("app.services.vector_store.OllamaEmbeddings") as mock:
...
class TestVectorStore:
    ...
    def test_add_chunks_stores_all_chunks(self, sample_chunks, mock_embeddings):
        store = VectorStore(persist_path=":memory:")
        store.add_chunks(sample_chunks)
        assert store.count() == 3

    def test_add_chunks_is_idempotent(self, sample_chunks, mock_embeddings):
        store = VectorStore(persist_path=":memory:")
        store.add_chunks(sample_chunks)
        store.add_chunks(sample_chunks)
        assert store.count() == 3

    def test_search_returns_top_k_results(self, sample_chunks, mock_embeddings):
        store = VectorStore(persist_path=":memory:")
        store.add_chunks(sample_chunks)
        results = store.search("find user by id", top_k=2)
        assert len(results) <= 2

    def test_search_returns_code_chunks(self, sample_chunks, mock_embeddings):
        store = VectorStore(persist_path=":memory:")
        store.add_chunks(sample_chunks)
        results = store.search("authentication token", top_k=3)
        assert all(isinstance(r, CodeChunk) for r in results)

    def test_search_returns_empty_when_store_is_empty(self, mock_embeddings):
        store = VectorStore(persist_path=":memory:")
        results = store.search("anything", top_k=3)
        assert results == []

    def test_clear_removes_all_chunks(self, sample_chunks, mock_embeddings):
        store = VectorStore(persist_path=":memory:")
        store.add_chunks(sample_chunks)
        store.clear()
        assert store.count() == 0

    def test_get_indexed_files_returns_unique_file_paths(self, sample_chunks, mock_embeddings):
        store = VectorStore(persist_path=":memory:")
        store.add_chunks(sample_chunks)
